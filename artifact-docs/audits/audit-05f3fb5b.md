...
Issues reviewed: TLO-36 through TLO-45 (plus full history)
...
1. **TLO_API_KEY absent from auditor environment** — archetype patches have accumulated across 4 audit runs and zero have been submitted. This is the single highest-leverage fix available.
2. **Founding Engineer retry rate increasing** — TLO-43 (6 runs), TLO-36 (4 runs). Root cause: async/interactive UI bugs with no reproduce-first discipline.
...
## Run-by-run findings (TLO-36 to TLO-45)

| Issue | Runs | Agent | Pattern |
|-------|------|-------|---------|
| TLO-36 Fix self-review loop | 4 | Founding Engineer | System bug, no reproduce step documented |
| TLO-38 Redirect after create | 3 | Founding Engineer | Same scope as TLO-34/TLO-28, third attempt at same problem |
| TLO-40 Fix archetype path | 3 | Founding Engineer | Path resolution — resolved correctly but took 3 attempts |
| TLO-43 Cancel audit button | **6** | Founding Engineer | Highest non-test count. Async UI state with polling/cancel. No incremental approach. |
| TLO-45 QA verify policies default | 3 | QA Engineer | Chrome MCP not reflected in archetype, QA had to re-verify twice |

### TLO-43 (6 runs) deep read
...
### Redirect regression (TLO-27 → TLO-28 → TLO-34 → TLO-38)
...
**Recommended action:** Apply patches from decisions/archetype-patches-65ff2dbb.md manually, or ensure TLO_API_KEY is set before the next audit run.
...
- `decisions/api-key-blocker.md` — recurring TLO_API_KEY absence

---

## Feature requests (cannot submit without API key)

1. **Feature: expose TLO_API_KEY to auditor agent environment** — 4 consecutive audits have produced valid patches that couldn't be submitted. Priority: high.
2. **Feature: async UI pattern guidance** — add a recommended pattern (polling → confirm → cancel) for interactive UI features to reduce retry loops like TLO-43.

---

## Stale docs check

- `security-audit-TLO-6.md` — root-level, no subdirectory. Low severity.
- `qa-ux-accessibility-TLO-13.md` — root-level, no subdirectory. Low severity.
...
- `product/ux-audit-TLO-11.md` — references may be stale post-Pico migration.
